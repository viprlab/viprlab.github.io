...
{% endif %}
{% if entry.internal %}{% if entry.type == "j" %}<p>test</p>{% else %}[{{entry.title}}]({{entry.url | prepend: site.baseurl}}){% endif %}{% else %} {% if entry.url %} [{{entry.title}}]({{entry.url}}){:target="_blank"} {% else %} [{{entry.title}}]({{entry.doi}}){:target="_blank"} {% endif %} {% endif %}<br />
