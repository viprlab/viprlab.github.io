...
{% endif %}
{% if entry.internal %}[{{entry.title}}]({{entry.url | prepend: site.baseurl}}){% else %} {% if entry.url %}<span>[{{entry.title}}]({{entry.url}}){:target="_blank"}{% if entry.type=='j' %}{: .journal}{% else %}{: .conf}{% endif %}</span>{: .abc}{% else %}<span>[{{entry.title}}]({{entry.doi}}){:target="_blank"}</span> {% endif %} {% endif %}<br />
