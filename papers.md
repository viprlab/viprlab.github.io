...
{% if entry.url %} &nbsp;[![pdf](/images/pdf-icon.png)]({{entry.url}}) {% endif %} {% if entry.doi %} [![doi](/images/doi-icon.png)]({{entry.doi}}) {% endif %} {% if entry.note %} [{{entry.note}}]{: style="color: maroon"} {% endif %}</span>
{% endfor %}
{% endfor %}
